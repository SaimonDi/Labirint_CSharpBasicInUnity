        ...
        public SavedData Load(string path = "")
            ...
            using (var reader = new XmlTextReader(path))
                ...
                while(reader.Read())
                    {
                    var key = "Name";
                    if(reader.IsStartElement(key)) result.Name = reader.GetAttribute("value");

                    key = "PosX";
                    if(reader.IsStartElement(key)) result.Position.X = reader.GetAttribute("value").TrySingle();

                    key = "PosY";
                    if(reader.IsStartElement(key)) result.Position.Y = reader.GetAttribute("value").TrySingle();

                    key = "PosZ";
                    if(reader.IsStartElement(key)) result.Position.Z = reader.GetAttribute("value").TrySingle();

                    key = "IsEnable";
                    if(reader.IsStartElement(key)) result.IsEnabled = reader.GetAttribute("value").TryBool();
                    }
                }
            return result;
            }
